...
module.exports = function (req, res) {
    ...
    try{
        ...
        checkParameter(param, 'weight')

        mysqlUtil.connectPool( async function (db_connection) {
            req.innerBody = {};

            req.innerBody['list'] = await queryCreate(req, db_connection);
            console.log(req.innerBody);
            if(!req.innerBody['list']){
                let err = new Error();
                err.code = '400';
                err.message = '좋아하는 영화 추가를 실패하였습니다.';
                res.send(err);
                return;
            }
            console.log(req.innerBody);
...
function queryCreate(req, db_connection){
    return mysqlUtil.queryArray(db_connection
        , 'call proc_create_user_liked_movie(?,?,?)'
        , [
            req.paramBody['movie_id'],
            req.paramBody['customer_id'],
            req.paramBody['weight'],
        ]
    );
}